// ==UserScript==
// @name         Netflix AI 雙語字幕 (v2.0.1)
// @version      2.0.1
// @description  高級標籤隔離技術，解鎖文本選取，支援遠端專有名詞庫 (Glossary JSON)。
// @author       Gemini
...
(function() {
    ...
    window.glossaryPrompt = ""; // 儲存轉換後嘅名詞表
            ...
            onload: function(res) {
                try {
                    ...
                    if (Array.isArray(arr) && arr.length > 0) {
                        ...
                        console.log("%c[v2.0.1] Glossary Loaded Successfully", "color: #00FF00;");
                    }
                } catch (e) {
                    console.error("[v2.0.1] Glossary JSON 解析失敗", e);
                }
            }
        });
    }
    fetchGlossary(); // 腳本載入時自動拉取
    ...
    // --- 4. 輸出 JSON 功能 ---
    function exportTranslationJSON(stats, mapping) {
        const outputData = {
        ...
        console.groupCollapsed("%c📺 Netflix AI Subtitles - JSON Export Data (v2.0.1)", "color: #00FF00; font-weight: bold; font-size: 12px;");
    ...
    const toggleLoading = (isTranslating, totalLines = 0) => {
        ...
        if (isTranslating) {
            ...
            window.uiTimer = setInterval(() => {
                ...
                loader.innerHTML = `
                    <div style="font-weight:bold; color:#FFD700; margin-bottom:8px; font-size:20px;">🚀 v2.0.1 標籤隔離翻譯中</div>
                    ...
                    <div style="font-size:11px; color:#888;">正在嚴格對齊 ${totalLines} 行數據</div>
                `;
            }, 1000);
    ...
    async function processAndTranslate(rawXml, url) {
        ...
        // 組合 System Prompt，加入 Glossary 數據
        const systemInstruction = `你是一位影視翻譯員。翻譯為「標準香港繁體中文（書面語）」。\n【對位死命令：標籤隔離模式】\n1. 你會收到格式為 "[id] 原文 [/id]" 的內容。\n2. 必須 1:1 對應每個 ID，禁止合併 ID，禁止跳過。\n3. 譯文必須被 ID 標籤包圍，格式："[id] 譯文 [/id]"。\n4. 嚴禁因為語法連貫而將下一行的語意提前。\n5. 符號、語氣詞必須保留。${window.glossaryPrompt}`;
        ...
        GM_xmlhttpRequest({
            ...
            data: JSON.stringify({
                ...
                messages: [{ role: "system", content: systemInstruction }, { role: "user", content: taggedInput }]
            }),
            onload: function(res) {
                try {
                    ...
                    while ((match = lineRegex.exec(aiContent)) !== null) {
                        ...
                        if (orig) {
                            ...
                            exportMapping.push({ id: idx, original: orig, translated: trans });
                        }
                    }

                    exportTranslationJSON({ model: db.activeModel, lines: originalLines.length, duration: duration }, exportMapping);
    ...
    function injectControlMenu() {
        ...
        wrapper.innerHTML = `
            <div class="${btnWrapper.className}"><button class="${targetBtn.className}" id="ai-toggle-btn" style="color:white; font-weight:bold; font-size:16px;">AI 2.0.1</button></div>
            <div id="ai-menu-popup" style="display:none; position:absolute; bottom:70px; left:50%; transform:translateX(-50%); background:rgba(10,10,10,0.98); border:1px solid #444; padding:20px; border-radius:10px; width:300px; flex-direction:column; gap:10px; z-index:2000002; color:white; box-shadow: 0 8px 24px rgba(0,0,0,0.9); font-size:14px;">
                ...
                <button id="ai-save-btn" style="background:#E50914; color:white; border:none; padding:10px; cursor:pointer; font-weight:bold; margin-top:10px; border-radius:4px;">儲存並套用 v2.0.1</button>
            </div>
        `;
